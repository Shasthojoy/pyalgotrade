...
class Database(dbfeed.Database):
    def __init__(self):
        ...
        self.__connection = mysql.connector.connect(user='root', password='123456',
                                      host='127.0.0.1',
                                      database='bovespa')
    ...
    def getBars(self, instrument, frequency, timezone=None, fromDateTime=None, toDateTime=None):
        ...
        for row in cursor:
            ...
            ret.append(bar.BasicBar(dateTime, float(row[1]), float(row[2]), float(row[3]), float(row[4]), row[5], row[6], row[7]))
        cursor.close()
        return ret
...
class Feed(membf.BarFeed):
    def __init__(self, maxLen=None):
        super(Feed, self).__init__(maxLen)
